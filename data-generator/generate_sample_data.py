...
def main():
    ...
    try:
        ...
        if existing_patients > 0:
            logger.warning(f"\nExisting data detected: {existing_patients} patients found in database. ")
             # Auto-clear in non-interactive environments (Docker, CI/CD, etc.)
            if not sys.stdin.isatty() or os.environ.get('AUTO_CLEAR_DATA'):
                logger.info("Non-interactive mode - auto-clearing existing data")
                clear_existing = True
            else:
